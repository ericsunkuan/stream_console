...
class ArenaOrchestrator extends EventEmitter {
  ...
  stop() {
    // Tear down both sessions and finalize WAV files
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('message', (data) => {
        ...
        if (msg.type === 'session.created') {
          ...
          // configure for audio-only streaming
        ...
        if (msg.type === 'response.audio_chunk' || msg.type === 'response.audio.delta') {
          // pick the correct Base64 payload
        ...
        // 3. all other events → log (transcripts, VAD, lifecycle, etc.)
        this.emit('log', {
          session: label,
          event: msg.type,
          payload: msg,
